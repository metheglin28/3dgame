...
func _physics_process(delta: float) -> void:
	...
	for id in player_nodes:
		...
		snapshot["players"][id] = {"pos": p.global_position, "rot": p.mesh.rotation.y, "hat": p.wearing_hat, "helmet": p.wearing_helmet, "cowboy": p.wearing_cowboy_hat, "bunny": p.wearing_bunny_ears, "wizard": p.wearing_wizard_hat, "golden": p.wearing_golden_ears, "tumble": p.tumble, "spec": p.spectating, "slow": p.is_slowed(), "riding": p.riding}
